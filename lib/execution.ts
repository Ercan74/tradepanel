...
export async function executeAiDecision(
  ...
  try {
    ...
    if (type === "REDUCE") {
      // REDUCE EMEKLİ EDİLDİ (2026-08-19). Kâr-alma artık TAMAMEN mekanik modelin
      // işi: risk-monitor TP1'de %25 satar, kalanı sürekli ATR-makas trailing ile
      // koşturur. Agent'ın iradi kısmi-satış yetkisi kaldırıldı — her cron turunda
      // tekrarlayan "kalanın %50'sini sat" REDUCE döngüsü kazananları TP1'den önce
      // doğruyor, "kazananı koşturma" disiplinini eziyor ve trailing'in gerçek
      // performansını ölçülemez kılıyordu (bkz. 2026-08-19 SOKM/ULKER incelemesi).
      // Bu, agent üretmese de kuyrukta kalmış eski APPROVED REDUCE'ları güvenle
      // reddeden savunma katmanıdır; kısmi satış YAPILMAZ.
      return {
        ok: false,
        message: `${decision.symbol} REDUCE emekli edildi — kısmi kâr-alma mekanik modele (TP1 %25 + trailing) bırakıldı, işlem yapılmadı.`,
      };
    }
