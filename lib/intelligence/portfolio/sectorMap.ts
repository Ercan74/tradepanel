/**
 * TIOS Intelligence Engine — Static BIST Sector Map (fallback)
 *
 * Source: Borsa İstanbul resmi sektör sınıflandırması (Sector Categories)
 * Compiled: 2026-06-30. Symbol count: 631
 *
 * This file replaces an earlier, smaller İş Yatırım-sourced map (113 symbols)
 * with the full official BIST sector classification, covering 631 symbols
 * across 48 distinct sectors.
 *
 * Resolution note: the source file lists each symbol under TWO category
 * levels — a broad umbrella (e.g. "MALİ KURULUŞLAR") and a specific
 * sub-category (e.g. "BANKALAR"). These are hierarchical, not conflicting:
 * every symbol's specific sub-category is a strict subset of exactly one
 * umbrella category. This map keeps only the SPECIFIC sub-category (the
 * smaller, more granular grouping) since that corresponds to the sector
 * level traders actually reason about and that BIST publishes as tradeable
 * sector indices (e.g. XBANK, XGIDA).
 *
 * Purpose
 * -------
 * This is a FALLBACK lookup, not the source of truth. The real sector
 * pipeline is: Matriks DDE (Kitap1.xlsx, column L) → matriks_excel_to_supabase.py
 * → Supabase `positions.sector` column. Once a position carries a live
 * `sector` value from Supabase, that value always wins over this map.
 *
 * Coverage: 631 BIST symbols — the official BIST sector classification,
 * which covers the large majority of listed companies. A symbol missing
 * from this map is genuinely unmapped (e.g. newly listed), not a bug.
 * Missing symbols fall through to UNKNOWN_SECTOR_LABEL exactly as before.
 */
...
/**
 * Looks up the static fallback sector for a BIST symbol.
 * Returns null if the symbol is not in the map — callers should treat
 * that as "genuinely unknown", not as an error.
 */
export function getStaticSector(symbol: string): string | null {
  const normalized = symbol.trim().toUpperCase();
  return BIST_SECTOR_MAP[normalized] ?? null;
}
